...
import { DdSdkReactNativeConfiguration, InitializationMode, addDefaultValuesToAutoInstrumentationConfiguration, buildConfigurationFromPartialConfiguration, formatFirstPartyHosts } from './DdSdkReactNativeConfiguration';
...
export class DdSdkReactNative {
  ...
  static initializeNativeSDK = async (configuration, params) => {
    ...
    await DdSdk.initialize(DdSdkReactNative.buildConfiguration(configuration, params));
  ...
  static setUserInfo = async userInfo => {
    InternalLog.log(`Setting user ${JSON.stringify(userInfo)}`, SdkVerbosity.DEBUG);
  ...
  static addUserExtraInfo = async extraUserInfo => {
    InternalLog.log(`Adding extra user info ${JSON.stringify(extraUserInfo)}`, SdkVerbosity.DEBUG);
  ...
  static buildConfiguration = (configuration, params) => {
    ...
    configuration.additionalConfiguration[DdSdkReactNative.DD_SDK_VERSION] = sdkVersion;
    ...
    }, configuration.nativeViewTracking, configuration.nativeInteractionTracking, configuration.verbosity, configuration.proxyConfig, configuration.serviceName, formatFirstPartyHosts(configuration.firstPartyHosts), configuration.bundleLogsWithRum, configuration.bundleLogsWithTraces, configuration.trackNonFatalAnrs, configuration.appHangThreshold, configuration.resourceTracingSamplingRate, configuration.trackWatchdogTerminations, configuration.batchProcessingLevel, configuration.initialResourceThreshold);
  };
