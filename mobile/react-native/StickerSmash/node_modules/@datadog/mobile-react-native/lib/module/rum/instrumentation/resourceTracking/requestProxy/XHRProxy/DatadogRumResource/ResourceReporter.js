...
const formatResourceStartContext = tracingAttributes => {
  ...
  if (tracingAttributes.samplingPriorityHeader !== '0') {
    attributes['_dd.span_id'] = tracingAttributes.spanId.toString(TracingIdFormat.decimal);
    attributes['_dd.trace_id'] = tracingAttributes.traceId.toString(TracingIdFormat.paddedHex);
...
const formatResourceStopContext = (timings, graphqlAttributes) => {
  ...
  if (timings.responseStartTime !== undefined) {
    attributes['_dd.resource_timings'] = createTimings(timings.startTime, timings.responseStartTime, timings.stopTime);
  }
...
const reportResource = async resource => {
  ...
  DdRum.stopResource(resource.key, resource.response.statusCode, resource.request.kind, resource.response.size, formatResourceStopContext(resource.timings, resource.graphqlAttributes), resource.timings.stopTime, resource.resourceContext);
};
