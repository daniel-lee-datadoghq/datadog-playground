...
exports.getDatadogExpoConfig = getDatadogExpoConfig;
exports.withDatadogDebugId = withDatadogDebugId;
exports.withDatadogMetroConfig = withDatadogMetroConfig;
var _metroSerializer = require("./metroSerializer");
 ...
 */
function withDatadogMetroConfig(config) {
 ...
 * @returns
 */
function getDatadogExpoConfig(projectRoot, options = {}) {
  const plugins = options.unstable_beforeAssetSerializationPlugins ?? [];
  const datadogOptions = {
    ...options,
    unstable_beforeAssetSerializationPlugins: [...plugins, _metroSerializer.unstable_beforeAssetSerializationPlugin]
  };
  return (options.getDefaultConfig ?? _utils.getDefaultExpoConfig)(projectRoot, datadogOptions);
}
 ...
 */
function withDatadogDebugId(config) {
  const customSerializer = (0, _metroSerializer.createDatadogMetroSerializer)(config.serializer?.customSerializer || undefined);
  return {
    ...config,
    serializer: {
      ...config.serializer,
