...
exports.UNKNOWN_TARGET_NAME = exports.DdEventsInterceptor = void 0;
...
const UNKNOWN_TARGET_NAME = exports.UNKNOWN_TARGET_NAME = 'unknown_target';
...
class DdEventsInterceptor {
  static ACTION_EVENT_DROPPED_DEBUG_MESSAGE = 'An action event was dropped because either the `onPress` method arguments' + ' were undefined or they were missing the target information.' + ' This log can happen if you use an abstraction component with an `onPress` prop' + ' that does not receive the native event is first argument.' + ' In this case, you can ignore this log.';
  debouncingStartedTimestamp = Number.MIN_VALUE;
  constructor(options = {}) {
  ...
  interceptOnPress(...args) {
    if (args.length > 0 && args[0] && args[0]._targetInst) {
      ...
      _InternalLog.InternalLog.log(DdEventsInterceptor.ACTION_EVENT_DROPPED_DEBUG_MESSAGE, _SdkVerbosity.SdkVerbosity.DEBUG);
    }
  }
  ...
  findClosestActionLabel(targetNode) {
    ...
    while (currentNode) {
      ...
      if (this.actionNameAttribute && props && props[this.actionNameAttribute]) {
